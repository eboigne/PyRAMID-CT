...
# Algorithm options:
# algorithm_name = 'Chambolle_Pock_tv' # Algorithm without any motion model (no PLI)
algorithm_name = 'Chambolle_Pock_tv_motion' # Algorithm with motion model (PLI)
...
case_name_prefix = 'test' # Prefix of the output folder
...
steady_iterations_first = 50 # Number of 'steady' iterations performed first before allowing dynamic motion.
...
skip_logger = False # Whether to add a logger Tee junction to split the log both in the console and log file
print_log_only = True # Whether to print the outputs in the log file only, or also in the console
skip_all_disk_saves = True # Whether to skip all saves on disk
...
np.random.seed(0)
mask_zero_path = np.random.rand(250,250) < 0.1 # Path of a 2D mask used to enforce 0 in the solution
mask_no_update_path = np.random.rand(250,250) < 0.1 # Path of a 2D mask used to enforce no update in the solution
mask_static_path = np.random.rand(250,250) < 0.1 # Path of a 2D mask used to enforce a static region in the solution
